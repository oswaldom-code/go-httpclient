...
func TestConstantBackoff(t *testing.T) {
	backoff := httpclient.ConstantBackoff(100 * time.Millisecond)
...
func TestLinearBackoff(t *testing.T) {
	backoff := httpclient.LinearBackoff(100*time.Millisecond, 500*time.Millisecond)
...
func TestExponentialBackoff_Max(t *testing.T) {
	backoff := httpclient.ExponentialBackoff(100*time.Millisecond, 500*time.Millisecond)
...
func TestFibonacciBackoff(t *testing.T) {
	backoff := httpclient.FibonacciBackoff(100*time.Millisecond, 10*time.Second)
...
func TestFibonacciBackoff_Max(t *testing.T) {
	backoff := httpclient.FibonacciBackoff(100*time.Millisecond, 500*time.Millisecond)
...
func TestDecorrelatedJitterBackoff(t *testing.T) {
	backoff := httpclient.DecorrelatedJitterBackoff(100*time.Millisecond, 10*time.Second)
...
func TestExponentialBackoffFullJitter(t *testing.T) {
	backoff := httpclient.ExponentialBackoffFullJitter(100*time.Millisecond, 10*time.Second)
...
func TestExponentialBackoffEqualJitter(t *testing.T) {
	backoff := httpclient.ExponentialBackoffEqualJitter(100*time.Millisecond, 10*time.Second)
...
func TestWithJitter(t *testing.T) {
	constant := httpclient.ConstantBackoff(100 * time.Millisecond)
	withJitter := httpclient.WithJitter(constant, 0.5) // 50% jitter
...
func TestWithMax(t *testing.T) {
	linear := httpclient.LinearBackoff(100*time.Millisecond, 10*time.Second)
	capped := httpclient.WithMax(linear, 300*time.Millisecond)
...
func TestWithMin(t *testing.T) {
	constant := httpclient.ConstantBackoff(10 * time.Millisecond)
	withMin := httpclient.WithMin(constant, 100*time.Millisecond)
...
func BenchmarkBackoffStrategies(b *testing.B) {
	strategies := map[string]httpclient.BackoffFunc{
		"Constant":    httpclient.ConstantBackoff(100 * time.Millisecond),
		"Linear":      httpclient.LinearBackoff(100*time.Millisecond, 10*time.Second),
		"Exponential": httpclient.ExponentialBackoff(100*time.Millisecond, 10*time.Second),
		"Fibonacci":   httpclient.FibonacciBackoff(100*time.Millisecond, 10*time.Second),
		"FullJitter":  httpclient.ExponentialBackoffFullJitter(100*time.Millisecond, 10*time.Second),
	}
