...
func TestRetry_SuccessOnFirstAttempt(t *testing.T) {
	...
	c := httpclient.New(
		...
		httpclient.WithMiddleware(httpclient.Retry(httpclient.RetryConfig{MaxAttempts: 3})),
	)
...
func TestExponentialBackoff(t *testing.T) {
	backoff := httpclient.ExponentialBackoff(100*time.Millisecond, 1*time.Second)
